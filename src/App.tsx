...
const TriggerNode: React.FC<NodeProps> = ({ data, selected }) => {
  return (
    <div
      className={`node-card rounded-xl px-4 py-3 w-[180px] border ${
        selected ? "border-primary shadow-glow" : "border-white/10"
      }`}
    >
...
const ActionNode: React.FC<NodeProps> = ({ data, selected }) => {
  ...
  return (
    <div
      className={`node-card rounded-xl px-4 py-3 w-[200px] border-l-4 ${
        selected ? "border-primary shadow-glow" : "border-white/10"
      }`}
...
const FlowCanvas: React.FC<{
...
}> = ({ selectedNodeId, onSelectNode, onUpdateNode }) => {
  ...
  const addWorkflowNode = useCallback(
    ...
    [onSelectNode, screenToFlowPosition, setNodes]
  );
  ...
  const updateSelectedNode = useCallback(
    (patch: Partial<WorkflowNodeData>) => {
      ...
      setNodes((prev) =>
        prev.map((node) => (node.id === selectedNode.id ? { ...node, data: { ...node.data, ...patch } } : node))
      );
      onUpdateNode(selectedNode.id, patch);
    },
    [onUpdateNode, selectedNode, setNodes]
  );
  ...
  return (
    <div className="absolute inset-0">
      ...
      <div className="absolute left-4 top-24 z-20">
        <div className="glass-panel rounded-xl border border-white/10 shadow-2xl p-3 flex flex-col gap-2">
          <button
            className="px-3 py-2 text-xs font-semibold text-white bg-slate-800 hover:bg-slate-700 rounded-lg border border-white/10 transition-colors"
            onClick={() =>
              addWorkflowNode("trigger", { label: "Manual Trigger", subtitle: "User initiated" })
            }
          >
            Add Trigger
          </button>
...
const App: React.FC = () => {
  ...
  const handleUpdateNode = useCallback((_nodeId: string, _patch: Partial<WorkflowNodeData>) => {
    // Placeholder for persistence or orchestrator integration.
  }, []);
        ...
        <aside className="w-72 glass-panel border-r border-t-0 flex flex-col shrink-0 z-20 absolute left-4 top-4 bottom-4 rounded-2xl shadow-2xl">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6 no-scrollbar">
            <section>
              ...
              <div className="p-3 bg-slate-800/40 border border-white/5 rounded-xl flex items-center gap-3">
            ...
            <section>
              ...
              <div className="space-y-2">
                <div className="p-3 bg-slate-800/40 border border-white/5 rounded-xl flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${palette.purple}`}>
                ...
                <div className="p-3 bg-slate-800/40 border border-white/5 rounded-xl flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${palette.warning}`}>
            ...
            <section>
              ...
              <div className="p-3 bg-slate-800/40 border border-white/5 rounded-xl flex items-center gap-3">
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${palette.blue}`}>
